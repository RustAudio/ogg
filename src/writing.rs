        ...
        for segment_i in 0..needed_segments {
            ...
            if segment_in_page_i == 0 {
                if segment_i + 1 < needed_segments {
                    ...
                    try!(PacketWriter::write_page(&mut self.wtr, serial, pg, false));
                } else {
                    // We have to write a page end, and its the very last
                    // we need to write
                    try!(PacketWriter::write_page(
                        &mut self.wtr,
                        serial,
                        pg,
                        is_end_stream
...
fn test_recapture() {
    ...
    let test_arr_3 = [3, 5, 9, 17, 33, 65, 129, 129, 127, 126, 125];
    {
        ...
        let ep = PacketWriteEndInfo::EndPage;
        {
            ...
            w.write_packet(Box::new(test_arr), 0xdeadb33f, ep, 0)
                .unwrap();
            ...
            w.write_packet(Box::new(test_arr_2), 0xdeadb33f, np, 1)
                .unwrap();
            w.write_packet(Box::new(test_arr_3), 0xdeadb33f, ep, 2)
                .unwrap();
        }
    }
