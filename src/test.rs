...
fn test_packet_rw() {
    ...
    let test_arr_3 = [3, 5, 9, 17, 33, 65, 129, 129, 127, 126, 125];
    {
        ...
        w.write_packet(&test_arr_3[..], 0xdeadb33f, PacketWriteEndInfo::EndPage, 2)
            .unwrap();
    }
...
fn test_page_end_after_first_packet() {
    ...
    let test_arr_3 = [3, 5, 9, 17, 33, 65, 129, 129, 127, 126, 125];
    {
        let mut w = PacketWriter::new(&mut c);
        let np = PacketWriteEndInfo::NormalPacket;
        w.write_packet(&test_arr[..], 0xdeadb33f, PacketWriteEndInfo::EndPage, 0)
            .unwrap();
        w.write_packet(&test_arr_2[..], 0xdeadb33f, np, 1).unwrap();
        w.write_packet(&test_arr_3[..], 0xdeadb33f, PacketWriteEndInfo::EndPage, 2)
            .unwrap();
...
fn test_packet_write() {
    ...
    let test_arr_out = [
        ...
        0xa3, 0x90, 0x5b, 0x00, 0x00, 0x00, 0x00, 0x6d, 0x94, 0x4e, 0x3d, 0x01, 0x1e, 0x01, 0x76,
        0x6f, 0x72, 0x62, 0x69, 0x73, 0x00, 0x00, 0x00, 0x00, 0x02, 0x44, 0xac, 0x00, 0x00, 0x00,
        0x00, 0x00, 0x00, 0x80, 0xb5, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0xb8, 0x01u8,
    ];
    let test_arr_in = [
        0x01, 0x76, 0x6f, 0x72, 0x62, 0x69, 0x73, 0x00, 0x00, 0x00, 0x00, 0x02, 0x44, 0xac, 0x00,
        0x00, 0x00, 0x00, 0x00, 0x00, 0x80, 0xb5, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0xb8, 0x01u8,
    ];

    {
        let mut w = PacketWriter::new(&mut c);
        w.write_packet(&test_arr_in[..], 0x5b90a374, PacketWriteEndInfo::EndPage, 0)
            .unwrap();
    }
    //print_u8_slice(c.get_ref());
...
fn test_write_large() {
    ...
    let mut w = PacketWriter::new(&mut c);
    w.write_packet(&test_arr, 0x5b90a374, PacketWriteEndInfo::EndPage, 0)
        .unwrap();
    //print_u8_slice(c.get_ref());
...
fn test_byte_seeking_continued() {
...

    {
        ...
        w.write_packet(gen_pck(2, 270_000), 0xdeadb33f, np, 2)
            .unwrap();
        off = w.get_current_offs().unwrap();
        w.write_packet(gen_pck(3, 270_000), 0xdeadb33f, np, 3)
            .unwrap();
        w.write_packet(gen_pck(4, 270_000), 0xdeadb33f, es, 4)
            .unwrap();
    }
    assert_eq!(c.seek(SeekFrom::Start(0)).unwrap(), 0);
...
fn test_issue_7() {
    ...
    {
        let mut w = PacketWriter::new(&mut c);
        w.write_packet(&test_arr[..], 0xdeadb33f, PacketWriteEndInfo::EndStream, 0)
            .unwrap();
    }
