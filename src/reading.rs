...
impl OggPage {
    ...
    fn has_whole_packet(&self) -> bool {
        self.0.bi.packet_positions.len().saturating_sub(
            self.0.bi.ends_with_continued as usize + self.0.bi.starts_with_continued as usize,
        ) > 0
    }
...
impl PageParser {
    ...
    pub fn new(header_buf: [u8; 27]) -> Result<(PageParser, usize), OggReadError> {
        ...
        if stream_structure_version != 0 {
            return Err(OggReadError::InvalidStreamStructVer(
    ...
    pub fn parse_segments(&mut self, segments_buf: Vec<u8>) -> usize {
        ...
        let mut packets =
            Vec::with_capacity(self.packet_count as usize + self.bi.ends_with_continued as usize);
    ...
    pub fn parse_packet_data(mut self, packet_data: Vec<u8>) -> Result<OggPage, OggReadError> {
        ...
        if self.checksum != hash_calculated {
            ...
            if !cfg!(fuzzing) {
                return Err(OggReadError::HashMismatch(self.checksum, hash_calculated));
            }
        }
            ...
            Entry::Occupied(mut o) => {
                ...
                if pg_prs.bi.starts_with_continued != inf.bi.ends_with_continued {
                    ...
                    if len as usize != inf.page_body.len() {
                        ...
                        inf.last_overlap_pck
                            .push(replace(&mut inf.page_body, vec![0; 0]));
                    }
                }
                inf.bi = pg_prs.bi;
                inf.packet_idx = 0;
...
impl<T: io::Read + io::Seek> PacketReader<T> {
    ...
    pub fn read_packet_expected(&mut self) -> Result<Packet, OggReadError> {
        Ok(self.read_packet()?.ok_or_else(|| {
            Error::new(
...
fn seek_before_end<T: io::Read + io::Seek>(mut rdr: T, offs: u64) -> Result<u64, OggReadError> {
    ...
    Ok(rdr.seek(SeekFrom::End(-(end_pos_to_seek as i64)))?)
}
